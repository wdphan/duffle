...
import { QueryClient, QueryClientProvider, useQueryClient as useQueryClient$1, useIsRestoring, useQueryErrorResetBoundary, notifyManager, InfiniteQueryObserver, parseMutationArgs, useMutation as useMutation$1, QueryObserver } from '@tanstack/react-query';
import { persistQueryClient } from '@tanstack/react-query-persist-client';
import { createClient as createClient$1, watchProvider, getProvider, watchWebSocketProvider, getWebSocketProvider, fetchBlockNumber, fetchFeeData, deepEqual, getAccount, watchAccount, fetchBalance, connect, disconnect, getNetwork, watchNetwork, watchSigner, fetchSigner, signMessage, signTypedData, switchNetwork, getContract, readContracts, parseContractResult, readContract, writeContract, prepareWriteContract, fetchToken, fetchEnsAddress, fetchEnsAvatar, fetchEnsName, fetchEnsResolver, prepareSendTransaction, sendTransaction, fetchTransaction, waitForTransaction } from '@wagmi/core';
export { AddChainError, ChainDoesNotSupportMulticallError, ChainMismatchError, ChainNotConfiguredError, Connector, ConnectorAlreadyConnectedError, ConnectorNotFoundError, ContractMethodDoesNotExistError, ContractMethodNoResultError, ContractMethodRevertedError, ContractResultDecodeError, ProviderChainsNotFound, ProviderRpcError, ResourceUnavailableError, RpcError, SwitchChainError, SwitchChainNotSupportedError, UserRejectedRequestError, alchemyRpcUrls, allChains, chain, chainId, configureChains, createStorage, deepEqual, defaultChains, defaultL2Chains, erc20ABI, erc4626ABI, erc721ABI, etherscanBlockExplorers, infuraRpcUrls, publicRpcUrls, readContracts } from '@wagmi/core';
...
function createClient(_ref) {
  ...
  if (persister) persistQueryClient({
    ...
    persister,
    dehydrateOptions: {
      shouldDehydrateQuery: query => query.cacheTime !== 0 && // Note: adding a `persist` flag to a query key will instruct the
...
const useQueryClient = () => useQueryClient$1({
  context: queryClientContext
});
...
function useBlockNumber() {
  ...
  }, [chainId, scopeKey, onBlock, provider, queryClient, watch, webSocketProvider]);
...
function useConnect() {
  ...
  const connect = React.useCallback(args => {
    return mutate({
      chainId: (args === null || args === void 0 ? void 0 : args.chainId) ?? chainId,
      connector: (args === null || args === void 0 ? void 0 : args.connector) ?? connector
    });
...
function useDisconnect() {
  ...
  const {
    ...
    reset,
    status
  } = useMutation(mutationKey$5, mutationFn$5, { ...(onError ? {
...
function useSigner() {
  ...
  const signerQuery = useQuery(queryKey$c({
    chainId
  }), queryFn$c, {
    cacheTime: 0,
    staleTime: Infinity,
  ...
  React.useEffect(() => {
    const unwatch = watchSigner({
      chainId
    }, signer => queryClient.setQueryData(queryKey$c({
      chainId
    }), signer));
    return unwatch;
  }, [queryClient, chainId]);
...
function useSignTypedData() {
  ...
  const signTypedData = React.useCallback(args => mutate({
    domain: (args === null || args === void 0 ? void 0 : args.domain) ?? domain,
    types: (args === null || args === void 0 ? void 0 : args.types) ?? types,
    value: (args === null || args === void 0 ? void 0 : args.value) ?? value
  }), [domain, types, value, mutate]);
  const signTypedDataAsync = React.useCallback(args => mutateAsync({
    domain: (args === null || args === void 0 ? void 0 : args.domain) ?? domain,
    types: (args === null || args === void 0 ? void 0 : args.types) ?? types,
    value: (args === null || args === void 0 ? void 0 : args.value) ?? value
...
function useContractReads() {
  ...
  const enabled = React.useMemo(() => {
    let enabled = Boolean(enabled_ && (contracts === null || contracts === void 0 ? void 0 : contracts.every(x => x.abi && x.address && x.functionName)));
    if (cacheOnBlock) enabled = Boolean(enabled && blockNumber);
    return enabled;
  ...
  const abis = (contracts ?? []).map(_ref5 => {
    let {
      abi
    } = _ref5;
    return abi;
    ...
    select(data) {
      const result = data.map((data, i) => {
        const {
          abi,
          functionName
        } = (contracts === null || contracts === void 0 ? void 0 : contracts[i]) ?? {};
...
function useContractWrite() {
  ...
  const write = React.useCallback(overrideConfig => {
    return mutate({
      address,
      args: (overrideConfig === null || overrideConfig === void 0 ? void 0 : overrideConfig.recklesslySetUnpreparedArgs) ?? args,
      chainId,
      abi,
      functionName,
      mode: overrideConfig ? 'recklesslyUnprepared' : mode,
      overrides: (overrideConfig === null || overrideConfig === void 0 ? void 0 : overrideConfig.recklesslySetUnpreparedOverrides) ?? overrides,
      request
    });
...
function useSendTransaction(_ref2) {
  ...
  const sendTransaction = React.useCallback(args => mutate({
    chainId,
    mode,
    request: (args === null || args === void 0 ? void 0 : args.recklesslySetUnpreparedRequest) ?? request
